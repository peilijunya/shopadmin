      ...
      <el-main class="main">
        <div class="header">
          <div id="some-element1">
            <i :class="pictureStyle" @click="asideSqFn"></i>
          </div>
          <!-- 面包屑 -->
            ...
            <el-breadcrumb separator-class="el-icon-arrow-right" class="miaobaoxie">
              <el-breadcrumb-item>
                <i
                  ...
                  style="font:16px/18px '';font-weight:800 "
                >首页</i>
              </el-breadcrumb-item>
              <el-breadcrumb-item
          ...
          <div class="btn-fullscreen">
            ...
            <div @click="handleFullScreen">
              <el-tooltip effect="dark" :content="fullscreen ? `取消全屏`:`全屏`" placement="bottom">
                <i class="iconfont icon-quanping" id="some-element3" v-if="!fullscreen"></i>
...
import { postImg, getMenus } from "@/api";
export default {
  filters: {
    fiterStrFn(data) {
      if (!isNaN(data)) {
  ...
  created() {
    this.imageUrl = localStorage.getItem("imageUrlSave");
    // 左侧导航菜单
  ...
  methods: {
    ...
    uploadFile(event) {
      ...
      reader.onload = arg => {
        document.querySelector(
          "#lable"
        ).style.backgroundImage = `url(${arg.target.result})`;
      };
    },
    // 点击删除上传图片
    delUplosdFn() {
    ...
    uploadImg() {
      ...
      postImg(this.imageParam).then(res => {
        ...
        if (res.meta.msg == 200) {
          this.imageUrl = res.meta.status.url;
          localStorage.setItem("imageUrlSave", res.meta.status.url);
          this.imageUnlode = false;
          this.$notify({
            duration: 2000,
            title: "成功",
